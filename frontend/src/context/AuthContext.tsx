...
export interface AuthUser {
  ...
  assessmentRemarks: Record<string, string[]>;
...
interface AuthContextType {
  ...
  issueCertificate: (courseId: string, score: number, level: string) => Promise<void>;
}
...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const issueCertificate = async (courseId: string, score: number, level: string) => {
    try {
      const res = await axios.post(`${API_BASE}/auth/issue-certificate`, { courseId, score, level });
      if (res.data.success) {
  ...
  return (
    <AuthContext.Provider value={{
      ...
      signup, login, logout, updateProfile, issueCertificate
    }}>
      {children}
