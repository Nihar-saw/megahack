...
import { Target, Zap, ChevronRight, ChevronLeft, CheckCircle2, Bot, Sparkles, Star, Award } from 'lucide-react';
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const res = await axios.post('http://localhost:5001/api/evaluate', {
        courseId,
        day: currentDay,
        task_scenario: currentAssessment.task,
      ...
      // Save to Auth Context & DB
      const finalScore = evaluation.career_readiness_score;
      const currentScores = user?.performanceScores?.[courseId] || [];
      const newScores = [...currentScores, finalScore];

      if (currentDay === 7 && evaluation.career_readiness_score) {
        await issueCertificate(courseId, finalScore, evaluation.level);
      }

      // Save remarks as a summary of the advanced evaluation
      const currentRemarks = user?.assessmentRemarks?.[courseId] || [];
      const combinedRemark = `Level: ${evaluation.level} | Score: ${finalScore}% | Strengths: ${evaluation.strengths.join(', ')}`;
      const newRemarks = [...currentRemarks, combinedRemark];
      ...
      const nextProgress = currentDay < 7 ? currentDay + 1 : 7;
      ...
      setEvaluationFeedback({
        ...
        strengths: [],
        improvement_areas: [],
        feedback: "The AI evaluator encountered an error while processing your response.",
      ...
      <div className="max-w-6xl mx-auto space-y-12">
          ...
          <div className="lg:col-span-12 space-y-8">
            ...
            >
              ...
              <div className="space-y-6">
                ...
                />
                <div className="flex flex-col md:flex-row gap-6 border-t border-slate-100 pt-8 mt-4">
                  <div className="flex-1">
                    {/* Render Feedback if Available */}
                    {isSubmitted && evaluationFeedback && (
                      <motion.div
                        initial={{ opacity: 0, scale: 0.95 }}
                        animate={{ opacity: 1, scale: 1 }}
                        className="p-8 bg-slate-900 rounded-[2.5rem] border border-slate-700 shadow-2xl relative overflow-hidden"
                      >
                        ...
                        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-10 relative z-10">
                          <div className="flex items-center gap-4">
                            ...
                            <div>
                              <h4 className="text-white font-black text-2xl tracking-tight">Career Readiness Report</h4>
                              <p className="text-blue-400/60 text-[10px] font-black uppercase tracking-[0.2em]">AI Performance Analysis</p>
                            </div>
                          </div>
                          <div className="flex items-center gap-4 bg-white/5 p-4 rounded-3xl border border-white/10 shadow-inner">
                            <div className="text-right">
                              <div className="text-[10px] text-slate-400 font-black uppercase tracking-widest mb-1">AI Rating</div>
                              <div className="text-xl font-black text-white">{evaluationFeedback.overall_rating_out_of_10}/10</div>
                            </div>
                            <div className="h-10 w-px bg-white/10" />
                            <div className="flex flex-col items-center">
                              <span className="text-[28px] font-black text-blue-400 leading-none">{evaluationFeedback.career_readiness_score}%</span>
                              <span className="text-[10px] text-slate-500 font-black uppercase tracking-[0.2em] mt-1">Ready</span>
                            </div>
                          </div>
                        </div>
                        ...
                        <div className="space-y-8 relative z-10">
                          ...
                          <div className="grid grid-cols-2 lg:grid-cols-3 gap-4">
                            {Object.entries(evaluationFeedback.skill_scores).map(([skill, score]) => (
                              <div key={skill} className="bg-white/5 border border-white/10 p-4 rounded-3xl group hover:bg-white/10 transition-all">
                                <div className="text-[10px] text-slate-400 uppercase tracking-widest mb-3 font-black group-hover:text-blue-400 transition-colors">
                                  {skill.replace('_', ' ')}
                                </div>
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div className="bg-emerald-500/10 border border-emerald-500/20 p-6 rounded-[2rem]">
                              <div className="flex items-center gap-2 mb-4 text-emerald-400">
                                ...
                                <strong className="uppercase tracking-widest text-[11px] font-black">Top Strengths</strong>
                              </div>
                              <ul className="space-y-3">
                                {evaluationFeedback.strengths.map((s, i) => (
                                  <li key={i} className="text-emerald-100/80 text-sm font-medium flex gap-3">
                                    <span className="text-emerald-500 font-black">0{i+1}</span> {s}
                                  </li>
                                ))}
                              </ul>
                            </div>
                            <div className="bg-rose-500/10 border border-rose-500/20 p-6 rounded-[2rem]">
                              <div className="flex items-center gap-2 mb-4 text-rose-400">
                                <Zap className="w-5 h-5" />
                                <strong className="uppercase tracking-widest text-[11px] font-black">Growth Areas</strong>
                              </div>
                              <ul className="space-y-3">
                                {evaluationFeedback.improvement_areas.map((s, i) => (
                                  <li key={i} className="text-rose-100/80 text-sm font-medium flex gap-3">
                                    <span className="text-rose-500 font-black">0{i+1}</span> {s}
                                  </li>
                                ))}
                              </ul>
                            </div>
                          ...
                          <div className="space-y-4">
                            <div className="bg-white/5 border border-white/10 p-6 rounded-[2rem]">
                              <strong className="text-blue-400 block mb-3 uppercase tracking-widest text-[11px] font-black">Expert Feedback</strong>
                              <p className="text-slate-300 text-base leading-relaxed font-medium">{evaluationFeedback.feedback}</p>
                            </div>
                            
                            <div className="bg-indigo-600/20 border border-indigo-500/30 p-8 rounded-[2rem]">
                              <strong className="text-indigo-400 block mb-4 uppercase tracking-widest text-[11px] font-black">Action Plan</strong>
                              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                {evaluationFeedback.recommendations.map((rec, i) => (
                                  <div key={i} className="bg-white/5 border border-white/10 p-4 rounded-2xl text-indigo-100 text-xs font-bold leading-relaxed shadow-inner">
                                    {rec}
                                  </div>
                                ))}
                              </div>
                          ...
                          {currentDay === 7 && isSubmitted && !isEvaluating && (
                            <motion.div 
                              initial={{ opacity: 0, scale: 0.95 }}
                              animate={{ opacity: 1, scale: 1 }}
                              className="mt-8 p-8 bg-gradient-to-br from-blue-600 to-indigo-700 rounded-[2.5rem] shadow-2xl shadow-blue-500/20"
                            >
                              <div className="flex items-center gap-6 mb-6">
                                <div className="w-16 h-16 bg-white/20 rounded-3xl flex items-center justify-center border border-white/30">
                                  <Bot className="w-8 h-8 text-white" />
                                </div>
                                <div className="text-white">
                                  <h5 className="font-black text-2xl tracking-tight">Final Step: AI Interview</h5>
                                  <p className="text-white/70 text-[10px] font-black uppercase tracking-[0.2em] mt-1">Market Readiness Validation</p>
                                </div>
                              </div>
                              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <Button
                                  onClick={() => navigate('/interview?course=' + courseId)}
                                  className="w-full bg-white text-indigo-700 h-16 rounded-2xl font-black shadow-xl hover:bg-slate-50 transition-all flex items-center justify-center gap-3 group text-lg"
                                >
                                  Start Live Interview <ChevronRight className="w-5 h-5 group-hover:translate-x-1 transition-transform" />
                                </Button>
                                {!user?.certificates?.some(c => c.courseId === courseId) && (
                                  <Button
                                    onClick={async () => {
                                      if (evaluationFeedback) {
                                        await issueCertificate(courseId, evaluationFeedback.career_readiness_score, evaluationFeedback.level);
                                        navigate('/certificates');
                                      }
                                    }}
                                    className="w-full bg-indigo-500 text-white h-16 rounded-2xl font-black shadow-xl hover:bg-indigo-400 transition-all flex items-center justify-center gap-3 group text-lg border-2 border-indigo-400"
                                  >
                                ...
                                {user?.certificates?.some(c => c.courseId === courseId) && (
                                  <Button
                                    ...
                                    className="w-full bg-emerald-600 text-white h-16 rounded-2xl font-black shadow-xl hover:bg-emerald-500 transition-all flex items-center justify-center gap-3 group text-lg"
                                  >
                                    <Award className="w-6 h-6" /> View Certificate
                                  </Button>
                                )}
                              </div>
                            </motion.div>
                  ...
                  <div className="flex flex-col justify-end gap-4 min-w-[280px]">
                    {!isSubmitted ? (
                      <Button
                        ...
                        className="w-full h-20 rounded-3xl bg-indigo-600 text-white text-lg font-black shadow-2xl shadow-indigo-200 hover:bg-indigo-700 active:scale-95 transition-all flex items-center justify-center gap-4 group disabled:opacity-70"
                      >
                        {isEvaluating ? (
                          <>
                      ...
                      <Button
                        onClick={currentDay === 7 ? () => window.scrollTo({ top: 0, behavior: 'smooth' }) : handleNextDay}
        ...
        <Modal isOpen={showRatingModal} onClose={() => setShowRatingModal(false)}>
          <div className="p-10 text-center">
            <div className="w-20 h-20 bg-indigo-600 rounded-[2rem] flex items-center justify-center mx-auto mb-8 shadow-xl shadow-indigo-100 border-4 border-white">
              <Sparkles className="w-10 h-10 text-white" />
            </div>
            
            <h3 className="text-3xl font-black text-slate-900 mb-2">AI Evaluation Complete!</h3>
            <p className="text-slate-400 font-bold text-sm tracking-widest uppercase mb-10">Your Performance Rating</p>

            <div className="bg-slate-50 rounded-[3rem] p-10 border-2 border-slate-100 mb-10 relative overflow-hidden group">
                ...
                <div className="flex flex-col items-center">
                  <div className="flex items-end gap-2 mb-2">
                    <span className="text-[64px] font-black text-slate-900 leading-none">
                      {evaluationFeedback?.overall_rating_out_of_10}
                    </span>
                    <span className="text-2xl font-black text-slate-400 mb-2">/10</span>
                  </div>
                  <div className="px-6 py-2 bg-indigo-600 text-white rounded-2xl font-black text-sm shadow-lg mb-6 tracking-wide">
                    Level: {evaluationFeedback?.level} (Score: {evaluationFeedback?.career_readiness_score}%)
                  </div>
                ...
                <div className="flex gap-2 mb-2">
                  {[1, 2, 3, 4, 5].map((star) => (
                    <Star 
                      key={star} 
                      className={`w-8 h-8 ${
                        star <= Math.round((evaluationFeedback?.career_readiness_score || 0) / 20)
            ...
            <div className="grid grid-cols-1 gap-4 mb-10">
              {evaluationFeedback?.strengths.slice(0, 2).map((s, i) => (
                <div key={i} className="flex items-center gap-4 bg-emerald-50/50 border border-emerald-100 p-5 rounded-2xl text-left hover:bg-emerald-50 transition-colors">
                  <div className="w-8 h-8 bg-emerald-500 rounded-xl flex items-center justify-center shrink-0">
                    <CheckCircle2 className="w-4 h-4 text-white" />
                  </div>
                  <span className="text-emerald-700 font-black text-xs leading-tight">{s}</span>
                </div>
              ))}
            </div>
            ...
            <Button 
              ...
              className="w-full h-16 bg-slate-900 text-white rounded-2xl font-black text-lg shadow-xl shadow-slate-200 hover:scale-[1.02] active:scale-95 transition-all"
            >
