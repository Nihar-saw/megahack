...
import { MoreVertical, FolderOpen, BarChart3, Clock, User, ArrowRight, Hash, Calendar, Star, Award } from 'lucide-react';
            ...
            <div className="flex gap-2">
              {status === 'Completed' && (
                <Link to="/certificates">
                  <Button variant="outline" className="border-emerald-500 text-emerald-600 hover:bg-emerald-50 rounded-xl px-4 py-2.5 text-xs font-black flex items-center gap-2 transition-all active:scale-95">
              ...
              <Link to={`/assessment?course=${courseId}`}>
                <Button className="bg-[#0f172a] hover:bg-slate-800 text-white rounded-xl px-6 py-2.5 text-xs font-black flex items-center gap-2 transition-all active:scale-95">
                  {status === 'Completed' ? 'Review' : 'Enter Simulation'} <ArrowRight className="w-4 h-4" />
                </Button>
              </Link>
            </div>
          )}
